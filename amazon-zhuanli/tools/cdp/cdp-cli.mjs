...
async function connectSession(config) {
  ...
  const browser = await chromium.connectOverCDP(session.endpoint, { noDefaults: true });
...
export function parseTrademarkCards(cardTexts) {
  const results = [];
  const labels = new Set(["wordmark", "status", "goods & services", "class", "serial", "owners", "outbound"]);
  const valuesAfter = (lines, label) => {
    const index = lines.findIndex((line) => line.toLowerCase() === label);
    if (index < 0) return [];
    const values = [];
    for (const line of lines.slice(index + 1)) {
      if (line.toLowerCase() === label) continue;
      if (labels.has(line.toLowerCase())) break;
      values.push(line);
    }
    return values;
  };
  for (const text of cardTexts) {
    ...
    const serial = valuesAfter(lines, "serial").find((value) => /^\d{8}$/.test(value))
      || lines.join(" ").match(/\b(?:Serial\s*)?(\d{8})\b/i)?.[1]
      || "";
    const markValues = valuesAfter(lines, "wordmark")
      .filter((value) => value.toLowerCase() !== "wordmark");
    const markText = markValues[markValues.length - 1] || "";
    if (!serial || !markText) continue;
    const status = valuesAfter(lines, "status").join(" ");
    const goods = valuesAfter(lines, "goods & services").filter((value) => value.toLowerCase() !== "outbound");
    const niceClasses = valuesAfter(lines, "class").filter((value) => /^\d{3}$/.test(value));
    const owner = valuesAfter(lines, "owners").join(" ");
    results.push({
      serial_number: serial,
      registration_number: "",
      mark_text: markText,
  ...
  return [...new Map(results.map((item) => [item.serial_number, item])).values()];
}
...
async function searchSemanticSnapshot(page, provider) {
  ...
  const cardTexts = provider === "uspto_tmsearch_browser"
    ? await page.locator(".result-card").allInnerTexts().catch(() => [])
    : [];
  const candidates = provider === "uspto_tmsearch_browser"
    ? (cardTexts.length ? parseTrademarkCards(cardTexts) : parseTrademarkRows(rows))
...
const PATENT_BASIC_GENERIC_WORDS = new Set([
  "a", "an", "and", "for", "in", "of", "on", "or", "the", "with",
  "black", "white", "outline", "pattern", "design", "image", "ornamental",
]);

function patentBasicKeywordTerms(query) {
  const tokens = String(query || "").match(/[A-Za-z0-9]+/g) || [];
  const meaningful = [];
  for (const token of tokens) {
    const normalized = token.toLowerCase();
    if (normalized.length < 3 || PATENT_BASIC_GENERIC_WORDS.has(normalized)) continue;
    if (!meaningful.includes(normalized)) meaningful.push(normalized);
  }
  if (!meaningful.length) throw new Error("USPTO Basic Search requires at least one meaningful keyword");
  return meaningful.length === 1
    ? meaningful
    : [meaningful[0], meaningful[meaningful.length - 1]];
}

export async function submitPatentBasicSearch(page, query) {
  const terms = patentBasicKeywordTerms(query);
  const first = page.locator("#searchText1");
  const second = page.locator("#searchText2");
  const operator = page.locator("#searchOperator");
  const button = page.locator("#basicSearchBtn");
  if (!await first.isVisible().catch(() => false)
      || !await second.isVisible().catch(() => false)
      || !await operator.isVisible().catch(() => false)
      || !await button.isVisible().catch(() => false)) {
    throw new Error("USPTO Patent Public Search Basic keyword fields are unavailable");
  }
  await page.locator("#searchField1").selectOption({ index: 0 });
  await first.fill(terms[0]);
  await operator.selectOption({ label: "AND" });
  await page.locator("#searchField2").selectOption({ index: 0 });
  await second.fill(terms[1] || "");
  await button.click();
  return terms;
}
...
async function runPlannedQuery(args, config) {
  ...
  let renderedQuery = provider === "uspto_tmsearch_browser"
    ? formatTmQuery(query.q, query.strategy)
    : String(query.q);
  let actionError = "";
  try {
    if (provider === "uspto_patent_browser") {
      renderedQuery = (await submitPatentBasicSearch(page, query.q)).join(" AND ");
    } else {
  ...
  const capture = {
    ...
    ...(provider === "uspto_tmsearch_browser"
      ...
      : { mode: query.mode || "basic_search", rendered_query: renderedQuery }),
...
export function parseTsdrRenderedText(text, requestedSerial) {
  const body = String(text || "");
  const value = (label) => body.match(
    new RegExp(`(?:^|\\n)${label}[ \\t]*:[ \\t]*([^\\n\\t]+)`, "i"),
  )?.[1]?.trim() || "";
  const values = (label) => [...body.matchAll(
    new RegExp(`(?:^|\\n)${label}[ \\t]*:[ \\t]*([^\\n]+)`, "gi"),
  )].map((match) => match[1].trim()).filter(Boolean);
  const pageCaseNumber = value("US Serial Number") || value("Serial Number") || value("Case Number");
  const markText = value("Mark") || value("Mark Literal Elements") || value("Word Mark");
  const caseStatus = value("Status") || value("Current Status");
  const owners = values("Owner Name");
  const goodsServices = values("For");
  const loading = /\bLOADING\b|正在处理/i.test(body);
  const identityMatches = cleanNumber(pageCaseNumber) === cleanNumber(requestedSerial);
  return {
    ready: !loading && identityMatches && Boolean(markText) && Boolean(caseStatus)
      && owners.length > 0 && goodsServices.length > 0,
    loading,
    page_case_number: pageCaseNumber,
    registration_number: value("US Registration Number") || value("Registration Number"),
    mark_text: markText,
...
export async function expandTsdrSections(page) {
  const requirements = [
    { label: "Goods and Services", pattern: /(?:^|\n)For[ \t]*:/i },
    { label: "Current Owner(s) Information", pattern: /(?:^|\n)Owner Name[ \t]*:/i },
  ];
  for (const requirement of requirements) {
    const bodyText = await page.locator("body").innerText().catch(() => "");
    if (requirement.pattern.test(bodyText)) continue;
    const link = page.locator("a.sectionLink").filter({ hasText: requirement.label }).first();
...
async function verifyCandidate(args, config) {
  ...
  if (provider === "uspto_tsdr") {
    ...
    await expandTsdrSections(page);
    tsdrSemantic = await waitForStableSemanticState(async () => {
      const refreshed = await freshState(page);
      const parsed = parseTsdrRenderedText(refreshed.bodyText, serial);
      const challenge = detectChallenge(refreshed.url, refreshed.title, refreshed.bodyText);
      const noResult = explicitNoResult(refreshed.bodyText);
      return {
        ready: challenge || noResult || parsed.ready,
        signature: JSON.stringify({
          url: refreshed.url,
          challenge,
          noResult,
    ...
    state = tsdrSemantic.state || await freshState(page);
    const parsed = tsdrSemantic.candidate || parseTsdrRenderedText(state.bodyText, serial);
    candidate = {
      serial_number: serial,
      page_case_number: parsed.page_case_number,
      registration_number: parsed.registration_number,
      mark_text: parsed.mark_text,
      case_status: parsed.case_status,
      owners: parsed.owners,
      goods_services: parsed.goods_services,
    };
  } else {
  ...
  if (challenge) {
    ...
    if (tsdrSemantic?.stable && !tsdrSemantic.timed_out && tsdrSemantic.candidate?.ready) status = "success";
    else if (explicitNoResult(state.bodyText)) status = "no_result";
    else detail = tsdrSemantic?.timed_out
      ? "TSDR identity/status/owner/goods fields did not become stable before timeout."
      : "TSDR identity/status/owner/goods fields could not all be confirmed from rendered content.";
  } else {
