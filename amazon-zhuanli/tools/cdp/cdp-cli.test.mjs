...
test("doctor can reconnect to the same dedicated Chrome session", () => {
  const runDoctor = () => spawnSync(
    process.execPath,
    ["./cdp-cli.mjs", "doctor"],
...
test("extracts trademark candidates from current USPTO result cards", async () => {
  const module = await import("./cdp-cli.mjs");
  assert.equal(
    typeof module.parseTrademarkCards,
    "function",
    "the TM Search adapter must parse the current rendered card layout",
  );
  const cards = [
    "Check to tag for 90976591\nWordmark\nwordmark\nCRAFTIKIT\nStatus\nLIVE\nREGISTERED\nGoods & services\nIC 016: Arts and craft clay kits; Arts and craft paint kits;...\noutbound\nClass\n016\nSerial\n90976591\nOwners\nCraftikit (LIMITED LIABILITY COMPANY; WYOMING, USA)",
  ];
  assert.deepEqual(module.parseTrademarkCards(cards), [{
    serial_number: "90976591",
    registration_number: "",
    mark_text: "CRAFTIKIT",
    owner: "Craftikit (LIMITED LIABILITY COMPANY; WYOMING, USA)",
    status: "LIVE REGISTERED",
    nice_classes: ["016"],
    goods_services: ["IC 016: Arts and craft clay kits; Arts and craft paint kits;..."],
  }]);
});
