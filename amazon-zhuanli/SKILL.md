...
- For JP, do not require or substitute Signa. Complete separate operator-confirmed patent/utility-model, design, and trademark recalls in J-PlatPat; accept official-registry evidence only from `j-platpat.inpit.go.jp` with a task-contained screenshot.
...
7. For US, run exact, phrase, and prefix trademark recall with `run-planned-query` in USPTO TM Search. Record the rendered query and distinct screenshot for each query/strategy, then verify material candidates with `verify-candidate` in TSDR. Use optional Signa/RapidAPI for phonetic/fuzzy cross-recall only. For JP, manually complete the three J-PlatPat recall operations `patent_recall`, `design_recall`, and `trademark_recall`; Signa is not a JP requirement. Use a distinct, readable image screenshot for each category. A JP `no_result` requires `operator_confirmed=true`, an empty candidates array, and the rendered result message; `not_applicable` never completes a JP recall. Treat every recorded JP recall candidate as material until a current candidate-level J-PlatPat verification resolves it. For EU and remaining non-US tasks where Signa is mandatory, probe Signa coverage first; EUIPO search is discovery until its official detail and image/view endpoints complete.
...
10. Record every operator-confirmed WIPO capture with `capture_transport=manual` and `operator_confirmed=true`. Record USPTO CDP captures with `capture_transport=cdp`, browser/protocol version, and sanitized session ID. Use `scripts/record_provider_result.py` for non-US manual official registries. JP records must use provider `official_registry_browser`, jurisdiction `JP`, evidence type `official_verification`, one of the three required recall operations, a J-PlatPat HTTPS result URL, and a distinct screenshot inside `screenshots/`. Preserve J-PlatPat-only candidates in the normalized payload so they enter candidate merging. Then normalize with `scripts/merge_candidates.py`.
...
- For JP, output no final risk grade unless J-PlatPat `patent_recall`, `design_recall`, and `trademark_recall` each finish with `success` or a rendered `no_result` capture.
