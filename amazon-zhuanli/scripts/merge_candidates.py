...
def patent_key(item: dict[str, Any]) -> str:
    ...
    number = str(
        publication or item.get("application_number") or item.get("grant_number")
        or item.get("registration_number") or item.get("record_number") or ""
    )
    ...
    for entry in entries:
        ...
        for row in rows:
            if not isinstance(row, dict) or not isinstance(row.get("official_verification"), dict):
                continue
            right_type = str(row.get("right_type") or "").casefold()
            if right_type == "trademark":
                row_kind = "trademark"
            elif right_type in {"patent", "utility_model", "design"}:
                row_kind = "patent"
            else:
...
def main() -> None:
    ...
    official_entries = [
        entry for entry in collections.get("official_verifications", [])
        if runs.get(str(entry.get("source_run_id")), {}).get("status") == "success"
    ]
    patents = merge("patent", [
        *collections.get("patents", []),
        *official_entries_for_kind(
            official_entries, "patent", {"uspto_patent_browser", "official_registry_browser"},
        ),
    ], runs)
    trademarks = merge("trademark", [
        *collections.get("trademarks", []),
        *official_entries_for_kind(
            official_entries, "trademark", {"official_registry_browser"},
        ),
    ], runs)
    official = verification_index(official_entries)
