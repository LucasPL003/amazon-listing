...
def jp_candidate_verification_complete(kind: str, item: dict[str, Any]) -> bool:
    """Require substantive J-PlatPat fields before a JP candidate is verified."""
    if str(item.get("jurisdiction") or "").upper() != "JP":
        return True
    verification = item.get("official_verification")
    if not isinstance(verification, dict) or verification.get("status") != "verified":
        return False
    source = str(verification.get("source") or "").casefold().replace("_", "-")
    parsed = urlparse(str(verification.get("url") or ""))
    hostname = (parsed.hostname or "").casefold()
    if source not in {"j-platpat", "jplatpat"} or parsed.scheme != "https":
        return False
    ...
    try:
        ...
        return False
    owner = item.get("owner") or item.get("owners") or verification.get("owner") or verification.get("owners")
    legal_status = item.get("legal_status") or item.get("status") or verification.get("legal_status")
    if not owner or not legal_status:
        return False
    right_type = str(item.get("right_type") or "").casefold()
    if kind == "trademark":
        identifier = item.get("application_number") or item.get("registration_number") or item.get("serial_number") or item.get("record_number")
        goods = item.get("goods_services") or item.get("nice_classes")
        return right_type == "trademark" and bool(identifier and item.get("mark_text") and goods)
    identifier = item.get("publication_number") or item.get("application_number") or item.get("grant_number") or item.get("record_number")
    if not identifier:
        return False
    if right_type == "design":
        views = item.get("views") or item.get("figures") or item.get("browser_evidence", {}).get("evidence_images")
        return bool(views)
    claims = item.get("claims") or item.get("claims_summary") or item.get("claim_text")
    return right_type in {"patent", "utility_model"} and bool(item.get("title") and claims)
...
def required_providers(jurisdictions: list[str]) -> list[str]:
    ...
    if any(value not in {"US", "JP"} for value in jurisdictions):
        providers.append("signa")
