...
from common import (
    ...
    ensure_object, jp_candidate_verification_complete, load_json, now_iso, sha256_json,
)
...
def material_unverified(candidates: dict[str, Any]) -> list[str]:
    ...
    for kind in ("patents", "trademarks"):
        for item in candidates.get(kind, []):
            ...
            if verified and str(item.get("jurisdiction") or "").upper() == "JP":
                verified = jp_candidate_verification_complete(kind[:-1] if kind.endswith("s") else kind, item)
            if item.get("material") and not verified:
...
def source_gaps(task: dict[str, Any], evidence: dict[str, Any], candidates: dict[str, Any], search_plan: dict[str, Any]) -> list[str]:
    ...
    for provider in task.get("required_sources", []):
        ...
        if provider == "official_registry_browser" and "JP" in {
            str(value).upper() for value in task.get("target_jurisdictions", [])
        }:
            required_operations = {"patent_recall", "design_recall", "trademark_recall"}
            jp_recalls = [
                run for run in provider_runs
                if str(run.get("jurisdiction") or "").upper() == "JP"
                and run.get("operation") in required_operations
                and run.get("status") in {"success", "no_result"}
                and run.get("evidence_type") == "official_verification"
                and run.get("request_params", {}).get("operator_confirmed") is True
                and str(run.get("request_params", {}).get("screenshot_sha256") or "")
                and (
                    run.get("status") != "no_result"
                    or str(run.get("request_params", {}).get("rendered_result_message") or "").strip()
                )
            ]
            completed_operations = {str(run.get("operation") or "") for run in jp_recalls}
            screenshot_hashes = {
                str(run.get("request_params", {}).get("screenshot_sha256") or "") for run in jp_recalls
            }
            if not required_operations.issubset(completed_operations) or len(screenshot_hashes) < 3:
                good = []
        if not good:
            gaps.append(provider)
    return sorted(set(gaps))
