...
def ops_get(path: str, range_header: str = "") -> tuple[bytes, dict[str, str]]:
    ...
    try:
        ...
        if (
            path.startswith("published-data/search?")
            and exc.code == "PROVIDER_HTTP_ERROR"
            and exc.http_status == 404
            and "SERVER.EntityNotFound" in exc.detail
            and "No results found" in exc.detail
        ):
            return exc.detail.partition("Provider HTTP 404: ")[2].encode("utf-8"), {}
