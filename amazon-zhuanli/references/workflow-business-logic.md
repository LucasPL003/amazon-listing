...
6. When Signa appears in `required_sources`, probe `/v1/offices`, select target offices, verify production availability (`live` or legacy `production`), sync date, record count, and a known-mark sample. A target office that fails any coverage test is `COVERAGE_UNVERIFIED` or `SOURCE_DATA_STALE`. JP never requires Signa.
7. For EU targets, require active subscriptions for both EUIPO Trademark Search and Design Search. For US targets, probe EPO OPS as a non-blocking low-risk gate and require visible Chrome CDP capability for Amazon, USPTO TM Search, TSDR, and Patent Public Search. WIPO remains an operator-confirmed manual checkpoint. Signa and RapidAPI are optional US cross-recall sources. The TSDR API is deliberately disabled. For JP, require a manual J-PlatPat checkpoint covering patent/utility-model, design, and trademark recall. For other jurisdictions, require a manual official-registry checkpoint.
...
`preflight.py --phase evidence --cdp-capability-confirmed` verifies the accepted Amazon capture, screenshot files, image format/hash/dimensions, explicit jurisdictions, jurisdiction/source routing, recent credential-preflight records, required Signa coverage where applicable, and visible Chrome CDP capability. JP has no Signa coverage gate. It records WIPO as manual and USPTO as CDP. Success moves the task to `collecting`. Legacy `2.1-free` tasks retain the old capability flags.
...
For JP, Signa is not a required source. The operator uses J-PlatPat to complete three distinct official-registry recalls: `patent_recall` for patents and utility models, `design_recall` for designs, and `trademark_recall` for word and figurative marks. Each recall records its query, rendered `j-platpat.inpit.go.jp` HTTPS URL, checked result, evidence type `official_verification`, and a distinct screenshot inside the task. `not_applicable` does not complete a recall. A zero result must be operator-confirmed and retain the rendered result message. Candidates found only in J-PlatPat remain in the normalized candidate merge. A missing category, reused screenshot, wrong evidence type, or unconfirmed zero result keeps the assessment `incomplete`; every material candidate still requires candidate-level official verification.
