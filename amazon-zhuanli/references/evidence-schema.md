...
For JP, record three separate `official_registry_browser` source runs through `record_provider_result.py`: `patent_recall`, `design_recall`, and `trademark_recall`. Each uses jurisdiction `JP`, evidence type `official_verification`, a rendered HTTPS URL whose host is `j-platpat.inpit.go.jp`, and a distinct readable screenshot image inside `screenshots/`; neither screenshot path nor content hash may be reused across categories. Each run must be `success` or a rendered, operator-confirmed `no_result`; `not_applicable` is invalid. A zero-result normalized object contains `operator_confirmed=true`, a non-empty `result_message`, and `candidates=[]`. A success object contains `operator_confirmed=true` and one or more candidates with `right_type`, jurisdiction, identifier, title or mark text, material flag, and an `official_verification` object. Recall candidates are forced to `material=true` until resolved. Failed or access-limited runs must not contain normalized candidates and are excluded from merging. J-PlatPat-only patent, utility-model, design, and trademark candidates are merged rather than discarded. All three recall operations are required even when no material candidate is found.

A material JP candidate is fully verified only when its J-PlatPat object has `status=verified`, J-PlatPat HTTPS URL, checked time within 48 hours, current owner, and legal status. A patent or utility model also needs a title and claims/claims summary; a design needs at least one official view/figure; a trademark needs mark text plus goods/services or Nice classes. Missing or stale fields keep the candidate in `unverified_material_candidates` and prohibit a final grade.
